...
def load_stop_event(work_dir):
    """Load stop event list."""
    stop_event_path = os.path.join(work_dir, "stoplist.txt")
    with open(stop_event_path, "r") as f:
        stop_events = f.read().splitlines()
    return stop_events


__all__ = ["count_stop_event", "load_stop_event"]
