...
class SingleChainSentModel(BasicModel):
    ...
    def pretrain(self):
        """Pretrain sentence prediction model."""
        work_dir = self._work_dir
        device = self._device
        npoch = 10
            ...
            for fn in sorted(os.listdir(tmp_dir)):
                ...
                with tqdm(total=len(train_set)) as pbar:
                    for iteration, (events, sents, masks, target) in enumerate(train_loader):
                        ...
                        model.train()
                        loss = model.forward_sent(sents, masks, target)
                        # Get loss
                        ...
                        if (iteration + 1) % interval == 0:
                            result = self.pretrain_evaluate(eval_set=dev_set, verbose=False)
                            if result > best_performance:
                                best_performance = result
                                self.save_model("pretrain")
                        # Update progress bar
                        pbar.set_description("Loss: {:.4f}".format(sum(batch_loss) / len(batch_loss)))
    ...
    def pretrain_evaluate(self, eval_set=None, verbose=True):
        ...
        with torch.no_grad():
            for events, sents, masks, target in eval_loader:
                ...
                masks = masks.to(device)
                target = target.to(device)
                pred = model.forward_sent(sents, masks)
            ...
            if True:
                ...
                with tqdm(total=len(train_set)) as pbar:
                    for iteration, (events, sents, masks, target) in enumerate(train_loader):
                        ...
                        target = target.to(device)
                        model.train()
                        event_loss, sent_loss, dist = model.forward_all(events, sents, masks, target)
                        loss = event_loss + sent_loss * lambda_sent + dist * lambda_dist
                        # Get loss
                        batch_loss.append(loss.item())
                        batch_event_loss.append(event_loss.item())
                        batch_sent_loss.append(sent_loss.item() * lambda_sent)
                        batch_constraint_loss.append(dist.item() * lambda_dist)
                        # Update gradient
                        optimizer.zero_grad()
                        loss.backward()
                        optimizer.step()
                        ...
                        pbar.set_description("event loss: {:.4f}, sent loss: {:.4f}, constraint: {:.4f}".format(
                            sum(batch_event_loss) / len(batch_event_loss),
                            sum(batch_sent_loss) / len(batch_sent_loss),
                            sum(batch_constraint_loss) / len(batch_constraint_loss)
                        ))
