...
from mcpredictor.models.single_chain.network import SCPredictorSent
            ...
            if True:
                ...
                with tqdm(total=len(train_set)) as pbar:
                    for iteration, (events, sents, masks, target) in enumerate(train_loader):
                        ...
                        model.train()
                        event_loss = model.forward_event(events, target)
                        loss = event_loss
                        # Get loss
                        ...
                        pbar.set_description("event loss: {:.4f}".format(event_loss.item()))
    ...
    def evaluate(self, eval_set=None, verbose=True):
        ...
        with torch.no_grad():
            for events, sents, masks, target in eval_loader:
                events = events.to(device)
                target = target.to(device)
                pred = model.forward_event(events)
