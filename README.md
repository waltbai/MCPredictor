...
Please follow  ```README.md``` and ```bin/event_pipeline/rich_docs/gigaword.txt``` to construct the dataset. ```bin/entity_narrative/eval/experiments/generate_sample.sh``` is used to generate dev/test dataset.
